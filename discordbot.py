...
@bot.event
async def on_ready():
    await bot.change_presence(activity=discord.Game(f"ヘルプは n:help | 導入サーバー数: {len(bot.guilds)}"))
...
async def newinfo(ctx):
    await embox("新着情報","\n**2020 7/18** 試験運用開始。\n**2020 7/18 **  BOTの稼働を開始しました。",0x77aa27,ctx.message)
...
async def on_message(message):
    ...
    if bot.user in message.mentions:
        ...
        await message.channel.send("あるひんけつもりのなかんちょう")
